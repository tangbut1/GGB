...
class LogManager:
    """Thread-safe forum log with optional monitor notification."""

    def __init__(self, task_id: str, log_dir: str = None,
                 monitor: Optional["ForumMonitor"] = None):
    ...
    def write(self, agent_name: str, iteration: int, content: str):
        ...
        # Notify monitor (event-driven, no polling)
        if self._monitor:
            self._monitor.on_message_written(agent_name)

    def set_monitor(self, monitor: "ForumMonitor"):
        """Wire the monitor after construction (avoids circular init)."""
        self._monitor = monitor
